...
public class AlunoServiceTest {
    ...
    void criarAluno_deveRetornarObjetoCriado() {
        ...
        alunoMock.setEmail(dto.email);
        alunoMock.setIdade(dto.idade);
        alunoMock.setRa(dto.ra);
        ...
        assertEquals(dto.email, alunoCriado.getEmail(), "O email deve ser igual ao fornecido no DTO");
        assertEquals(dto.idade, alunoCriado.getIdade(), "A idade deve ser igual à fornecida no DTO");
        assertEquals(dto.ra.getRa(), alunoCriado.getRa(), "O ra deve ser igual à fornecida no DTO");
    }
}
